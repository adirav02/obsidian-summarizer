...
Automated checks cover broken local image links, unclosed fences and display math, malformed Obsidian callout markers, forbidden `==highlight==` markup inside callouts, per-diagram Mermaid horizontal direction, required glossary-table formatting, table shape outside code fences, configured punctuation in prose including Callouts, and selected non-Latin linguistic scripts inside inline or display LaTeX. The Mermaid check uses explicit diagram metadata or the dominant direction of that block's labels rather than the note language. The LaTeX scan crosses lines and nested command content while allowing mathematical Unicode such as Greek symbols. These checks do not prove factual accuracy, pedagogy, renderer behavior, or whether a visual and section structure are appropriate; those still require qualitative review.
